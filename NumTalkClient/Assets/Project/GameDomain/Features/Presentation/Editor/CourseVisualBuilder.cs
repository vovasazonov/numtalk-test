...
namespace Project.GameDomain.Features.Presentation.Editor
{
    ...
    public static class CourseVisualBuilder
    {
        ...
        public static void Build()
        {
            ...
            _ice = Material("Glacier", new Color(0.30f, 0.80f, 0.94f), 0.48f);
            _amber = Material("CrumbleAmber", new Color(1f, 0.56f, 0.16f));
            ...
            var catalog = AssetDatabase.LoadAssetAtPath<CourseVisualCatalog>(Art + "/CourseVisualCatalog.asset");
            if (catalog == null) { catalog = ScriptableObject.CreateInstance<CourseVisualCatalog>(); AssetDatabase.CreateAsset(catalog, Art + "/CourseVisualCatalog.asset"); }
        ...
        private static Material Material(string name, Color color, float smoothness = 0.1f)
        {
        ...
        private static void BuildEffects(Transform parent)
        {
            ...
            effects.WeatherLabel = Label(canvas.transform, "Weather", "", new Vector2(0.5f, 1f), new Vector2(0, -86), new Vector2(850, 52), 25);
            effects.WeatherLabel.gameObject.SetActive(false);
        }

        private static Text Label(Transform parent, string name, string text, Vector2 anchor, Vector2 position, Vector2 size, int fontSize)
        {
        ...
        private static void StyleHud(Scene scene)
        {
        ...
        {
            const string path = Art + "/Heart.png";
            if (!File.Exists(path))
            {
